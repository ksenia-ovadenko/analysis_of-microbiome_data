...
NMDS + hierarchical clustering for family observation (genus level)
```{r, message=FALSE}
library("edgeR")
library("vegan")
library("factoextra")
...
Calculating farm scores for 2 clusters
```{r, warning=FALSE, message=FALSE, echo=FALSE}
...
# NB model with and DESeq2
## Data normalization and parameter estimation
...
## Diagnostic of p-values with histogram:

Next, the histogram of p-values is plotted. Under $H_o$ it should be uniformly distributed. Significant p-values will be visible as higher bar close to 0. 
...
Differential Abundance Testing for Hierarchical Clustering with 2 Groups. 
```{r, message=FALSE}
...
First we run GLM F test. The resulting table with top differently abundant genera can be seen below: 
```{r, message=FALSE, echo=FALSE}
...
After adjusting p-values with BH procedure, 515 genera were found significant:
```{r, message=FALSE, echo=FALSE}
...
Next we run GLM LRT test. The resulting table of top significant genera are below:
```{r, message=FALSE, echo=FALSE}
qlf_lrt <- glmLRT(fit, contrast=c(-1,1))
topTags(qlf_lrt)
```

Next we implement testing by fitting model with alternative design. 
First, GLM F-test is run with the results as can be seen below:
...
After adjusting for FDR by BH adjustment, 287 genera were found to have different abundunce:
```{r, message=FALSE, echo=FALSE}
...
The following top significant genera were detected with GLM LRT:
```{r, message=FALSE, echo=FALSE}
